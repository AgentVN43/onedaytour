...
const InfoTour = ({ info }) => {
    ...
    const handleUpdateUpdateOrder = async (values) => {
        ...
        const newOrder = {
            orderId: generatedId,
            customer: {
                name: values.customerName || info.customerName,
                phone: values.phone || info.phone,
                email: values.email || info.email,
                zalo: values.zalo || info.zalo,
                departing: values.departing || info.departing,
                arriving: values.arriving || info.arriving,
                departureDate: (values.date ? values.date[0].format("YYYY-MM-DD") : "") || info.date[0].format("YYYY-MM-DD"),
                returnDate: (values.date ? values.date[1].format("YYYY-MM-DD") : "") || info.date[1].format("YYYY-MM-DD"),
                passengers: values.passengers || info.passengers
            },
            quotes: null,
        ...
        message.success("Đơn hàng đã được cập nhậtnhật!");
    };
